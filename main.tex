...
\subsection{Overview}
The testbed is organized as three layers: a \emph{web frontend} for authoring and monitoring experiments, a \emph{backend} that orchestrates execution and safety, and a \emph{hardware layer} that actuates motion and measures force. This separation keeps the operator experience fast and clear while ensuring motion control and safety logic run reliably. 
...
E-Stop instantly disables motion; graceful Stop halts at the next safe point; Zero Sensor and calibration ensure consistent units; input validation and conservative defaults reduce operator error; Motor Check catches mechanical issues early.
...
\section{Operator Walkthrough: Using the Sequence Builder (Screenshots)}
...
Functional: Run/Stop/E-Stop, Motor Check, Zero/Calibrate, JSON Import/Export, Log Export.\\
Execution: Per-axis steps with movement-init, break, and optional hold.\\
Repeatability: Run the same sequence with Repeat to compare adhesion behavior.\\
Telemetry: Continuous $F_x, F_y, F_z$ (and shear where shown), step counts, and logs.\\
Safety: Force cap respected; E-Stop immediate; post-run reset to origin.
